...
class HrPayslip(Workflow, ModelSQL, ModelView):
    ...
    lines = fields.One2Many(
                'hr.payslip.line',
                'payslip', string='Payslip Lines', states={
            'readonly': ~Eval('state').in_(['draft'])
        },
        depends=['state'])
    # pay_and_allowance = fields.One2Many(
    #             'hr.payslip.line',
    #             'payslip_report', string='Payslip Lines',
    ...
    details_by_salary_rule_category = fields.One2Many(
            'hr.payslip.line', 'payslip_report',
    ...
    bank_address = fields.Text('Bank Address',states={
        ...
        depends=['state'], readonly= True)
    account_no = fields.Char('Account Number',states={
            'readonly': ~Eval('state').in_(['draft'])
        },
        depends=['state'], readonly= True)
    ...
    #         ], limit=1)
    #     if deduction:
    #         projected_tax_exemption = deduction[0].get_projected_tax_exemption()
    ...
    def on_change_with_fiscal_year(self):
        ...
        if self.month and self.year:
            ...
            if fiscal_years:
                current_fiscal_year = fiscal_years[0]
...
class SalaryStructure(ModelSQL, ModelView):
    ...
    def generate_pay_slip(self, employee, contract, payslip_batch=None):
        ...
        current_date = datetime.date(year, month_no, 1)
        Fiscal = Pool().get('account.fiscalyear')
        fiscal_years = Fiscal.search([
            ('start_date', '<=', current_date),
            ('end_date', '>=', current_date),
        ])
        if fiscal_years:
    ...
    def generate_pay_slip_for_ytd(self, employee, contract, given_month, payslip_batch=None):
        ...
        current_date = datetime.date(year, given_month, 1)
        Fiscal = Pool().get('account.fiscalyear')
        fiscal_years = Fiscal.search([
            ('start_date', '<=', current_date),
            ('end_date', '>=', current_date),
...
class SalaryBatch(ModelSQL, ModelView):
    ...
    def generate_payslip_batch_ytd(cls, records):
        ...
        for record in records:
            month_no = datetime.date.today().month
            year = datetime.date.today().year
            month = datetime.datetime(year, month_no, 1).strftime("%B")
            date_ = month +" " + str(year)
            # slipbatch = PayslipBatch.search([('name', '=', name)])
            # if not slipbatch:
    ...
    def batch_excel_workbook(cls, records):
        ...
        for record in records:
            for employee in record.employees:
                Payslip = Pool().get('hr.payslip')
                Payslips = Payslip.search([('employee', '=', employee.employee)])
                if Payslips != []:
                    PayslipLine = Pool().get('hr.payslip.line')
                    PayslipLines = PayslipLine.search([('payslip', '=', Payslips[0])])
